...
export default function Gameboard_x4() {
    ...
    }, [levelcounter, totalLevels, selectedLangs, modeOfTheBoard, router, refreshKey]);  //OLD: [levelcounter, totalLevels, navigate]);
    ...
    function checkFour(current)
          {
              ...
              if(items[current].id == items[prev].id && items[current].id == items[prevprev].id && items[current].id == items[prevprevprev].id )
              {
                ...
                setTimeout(()=>{
                  ...
                  router.push({pathname: "nextboard_x4",params: { moves, time }});
                //   stopTimer();
                //   resetTimer();
                  }
    ...
    <ScrollView style={{backgroundColor:"orange"}}>
        ...
        <View  style={{ marginLeft:-30, marginTop:60, marginBottom:10, flexDirection: "row", flexWrap: "wrap", justifyContent: 'center', alignItems: 'center'}}>
          <Link href="/" asChild>
            ...
            <Text style={globalStyles.landingHeader}>Game x4</Text>
        </View>
